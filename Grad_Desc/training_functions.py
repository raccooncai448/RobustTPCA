...
def generate_problem_videos(ranks, shape, alpha, kappa, device):
    images = []
    for filename in os.listdir("../../../Documents/MATLAB/Videezy4k/00"):
        img = cv2.imread(os.path.join("../../../Documents/MATLAB/Videezy4k/00",filename))
        if img is not None:
            images.append(img)
    images = np.array(images)
    print(images)
    X_star = torch.from_numpy(images).to(torch.float)
    Y = corrupt_X(X_star, alpha, device)
    return Y, X_star
