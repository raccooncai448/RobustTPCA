...
class TensorPCA:
    ...
    def r_t_pca(self, R):
        ...
        self.m_hat = {}
        for mode in range(self.order):
            rpca = R_pca(self.unfolded[str(mode)] @ self.unfolded[str(mode)].transpose())
            L, S = rpca.fit(max_iter=10000, iter_print=100)
            s, gamma = LA.eigh(L) # Eigen-decomposition
            self.s_hat[str(mode)] = np.sqrt(np.sort(s)[::-1][:R]).real  
            self.m_hat[str(mode)] = gamma[:,s.argsort()[::-1][:R]].real     
